  ...
  instructions: async (
    ...
    const languageRules = new LanguageRule({
      ...
      ja: [
        ...
        `  - If a word has trailing characters whose only purpose is to lengthen or dramatize (like "ぁ", "ー", "っ", "ぃ"), split these as a separate word and return the original standard word ` +
          `(e.g., "いやぁ" → "いや" + "ぁ" [original word: "いや"], "わぁ" → "わ" + "ぁ" [original: "わ"], "おはよううう" → "おはよう" + "うう" [original: "おはよう"], "すごーーい" → "すごい" [original: "すごい"]).`,

...
export class TextProcessingService {
  ...
  /**
   * Convert small and half-width katakana characters to their full-size equivalents
   * Normalizes katakana for consistent representation
   */
  private static normalizeKatakana(text: string): string {
    // Mapping of all small katakana (小書き文字) to big katakana
    const smallToBigKatakana: { [key: string]: string } = {
      '\u30A1': '\u30A2', // ァ → ア
      '\u30A3': '\u30A4', // ィ → イ
      '\u30A5': '\u30A6', // ゥ → ウ
      '\u30A7': '\u30A8', // ェ → エ
    ...
    const halfWidthToFullWidthKatakana: { [key: string]: string } = {
      '\uFF65': '\u30FB', // ･ → ・ (middle dot)
      '\uFF66': '\u30F2', // ｦ → ヲ
      '\uFF67': '\u30A1', // ｧ → ァ
      '\uFF68': '\u30A3', // ｨ → ィ
    ...
    const allKatakanaMap: { [key: string]: string } = {
      ...smallToBigKatakana,
    ...
    const voicedMarkMap: { [key: string]: string } = {
      '\uFF76\uFF9E': '\u30AC', // ｶﾞ → ガ
      '\uFF77\uFF9E': '\u30AE', // ｷﾞ → ギ
      '\uFF78\uFF9E': '\u30B0', // ｸﾞ → グ
      '\uFF79\uFF9E': '\u30B2', // ｹﾞ → ゲ
    ...
    // Convert all remaining small/half-width katakana characters to their full-width equivalents
    for (const [small, big] of Object.entries(allKatakanaMap)) {
      normalized = normalized.replace(new RegExp(small, 'g'), big);
    }
